...
void main() {
  group('Algorithm Step-by-Step Mode E2E Tests', () {
        ...
        overrides: [
          ...
          automatonSimulationProvider.overrideWith((ref) {
            return AutomatonSimulationNotifier(
              ref: ref,
              tracePersistenceService: null, // No persistence for tests
            );
          }),
    ...
    group('NFA→DFA Conversion Step-by-Step Mode', () {
      test('NFA→DFA conversion captures detailed subset construction steps',
          () async {
        ...
        expect(algorithmState.nfaToDfaStepsResult, isNotNull);
        ...
        for (int i = 0; i < totalSteps - 1; i++) {
          ...
          expect(state.canGoNext, i + 1 < totalSteps - 1);
          ...
          expect(currentStep.algorithmType, AlgorithmType.nfaToDfa);
        }
        ...
        expect(finalStepState.canGoNext, false);
        expect(finalStepState.canGoPrevious, true);
        ...
        for (int i = totalSteps - 1; i > 0; i--) {
          ...
          final state = container.read(algorithmStepProvider);
          expect(state.currentStepIndex, i - 1);
          expect(state.canGoPrevious, i - 1 > 0);
        ...
        expect(backToFirstState.canGoPrevious, false);
        expect(backToFirstState.canGoNext, true);

        // 5. Verify final result matches standard conversion
        final standardResult = await algorithmNotifier.convertNfaToDfa();
        expect(standardResult, isNotNull);
    ...
    group('DFA Minimization Step-by-Step Mode', () {
      test('DFA minimization captures detailed Hopcroft algorithm steps',
          () async {
        ...
        final algorithmState = container.read(automatonAlgorithmProvider);
        expect(algorithmState.error, isNull);
        expect(algorithmState.dfaMinimizationStepsResult, isNotNull);
        ...
        for (int i = 0; i < totalSteps - 1; i++) {
          ...
          final currentStep = state.currentStep;
          expect(currentStep, isNotNull);
          expect(currentStep!.algorithmType, AlgorithmType.dfaMinimization);
          expect(currentStep.title, isNotEmpty);
          expect(currentStep.explanation, isNotEmpty);
        }
        ...
        stepNotifier.setCurrentStep(totalSteps - 1); // Go to last step
      ...
      test('Minimization step explanations describe partition refinement',
          () async {
        // Create a simple DFA
        final stateNotifier = container.read(automatonStateProvider.notifier);
        final q0 = automaton_state.State(
        ...
        final hasPartitionMention = steps.any((step) =>
        ...
        expect(hasPartitionMention, isTrue,
            reason: 'Steps should explain partition refinement');
      });
    });

    group('FA→Regex Conversion Step-by-Step Mode', () {
      test('FA→Regex conversion captures detailed state elimination steps',
          () async {
        ...
        final algorithmState = container.read(automatonAlgorithmProvider);
        expect(algorithmState.error, isNull);
        expect(algorithmState.faToRegexStepsResult, isNotNull);
        ...
        for (int i = 0; i < totalSteps - 1; i++) {
          ...
          final currentStep = state.currentStep;
          expect(currentStep, isNotNull);
          expect(currentStep!.algorithmType, AlgorithmType.faToRegex);
          expect(currentStep.title, isNotEmpty);
          expect(currentStep.explanation, isNotEmpty);
        }
        ...
        expect(finalAlgorithmState.faToRegexStepsResult, isNotNull);

        // The result should contain a regex string
        final regexResult = finalAlgorithmState.faToRegexStepsResult!;
        expect(regexResult.regex, isNotEmpty);
        expect(regexResult.steps.isNotEmpty, isTrue);
      });

      test('Regex conversion step explanations describe state elimination',
          () async {
        // Create a simple FA
        final stateNotifier = container.read(automatonStateProvider.notifier);
        final q0 = automaton_state.State(
        ...
        final hasRegexMention = steps.any((step) =>
    ...
    group('Full Integration: All Three Algorithms', () {
      test(
        ...
        expect(finalAlgorithmState.nfaToDfaStepsResult, isNotNull);
        expect(finalAlgorithmState.dfaMinimizationStepsResult, isNotNull);
        expect(finalAlgorithmState.faToRegexStepsResult, isNotNull);

        // Verify final regex result
        expect(
          finalAlgorithmState.faToRegexStepsResult!.regex,
          isNotEmpty,
        );
      });
    });
