...
class LRParsing {
  ...
  static Map<int, LRState> buildLRAutomaton(ContextFreeGrammar grammar) {
    AlgoLog.startAlgo('lrAutomaton', 'Construção do Autômato LR(1)');
    AlgoLog.add('Construindo autômato LR(1)');
    
    final augmentedGrammar = createAugmentedGrammar(grammar);
    final firstSets = LLParsing.calculateFirstSets(augmentedGrammar);
    ...
    while (queue.isNotEmpty) {
      ...
      AlgoLog.add('Estado $stateId: ${currentItems.length} itens');
    ...
    AlgoLog.add('Autômato LR(1) construído com ${states.length} estados');
    return states;
  }

  /// Generate LR(1) parse table
  static LRParseTable generateParseTable(ContextFreeGrammar grammar) {
    AlgoLog.startAlgo('lrParseTable', 'Geração da Tabela de Parsing LR(1)');
    AlgoLog.add('Gerando tabela de parsing LR(1)');
    
    final augmentedGrammar = createAugmentedGrammar(grammar);
    final states = buildLRAutomaton(augmentedGrammar);
    final followSets = LLParsing.calculateFollowSets(augmentedGrammar);
    ...
    for (final stateEntry in states.entries) {
      ...
      AlgoLog.add('Processando estado $stateId com ${state.items.length} itens');
      
      for (final item in state.items) {
        AlgoLog.add('  Item: $item');
        ...
        if (item.isReduceItem) {
          ...
          if (item.lhs == augmentedGrammar.startVariable) {
            ...
            AlgoLog.add('  Estado $stateId: acc em \$');
          } else {
            ...
            if (productionNumber >= 0) {
              ...
              AlgoLog.add('  Estado $stateId: r$productionNumber em ${item.lookahead}');
            }
          }
        } else {
          // Shift item
            ...
            for (final stateEntry2 in states.entries) {
              if (stateEntry2.value.items == gotoItems) {
                ...
                if (grammar.isTerminal(symbolAfterDot)) {
                  ...
                  AlgoLog.add('  Estado $stateId: s$targetStateId em $symbolAfterDot');
                } else {
                  table.addGoto(stateId, symbolAfterDot, targetStateId);
                  AlgoLog.add('  Estado $stateId: goto $targetStateId em $symbolAfterDot');
                }
                break;
              }
            }
    ...
    while (true) {
      ...
      switch (action.action) {
        ...
        case 'r':
          ...
          // Pop 2 * |β| elements from stack (states and symbols)
