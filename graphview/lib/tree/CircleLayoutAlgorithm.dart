...
class CircleLayoutAlgorithm extends Algorithm {
  ...
  List<Node> _optimizeNodeOrder(Graph graph) {
    ...
    for (var attempt = 0; attempt < attempts; attempt++) {
      ...
      if (attempt > 0) {
        currentOrder = _rotatedOrder(currentOrder, attempt);
      }
  ...
  List<Node> _rotatedOrder(List<Node> nodes, int attempt) {
    final rotation = attempt % nodes.length;
  ...
  Size _layoutNodes(Graph graph) {
    final radius =
        _radius > 0 ? _radius : 0.35 * max(_layoutWidth, _layoutHeight);

    final centerX = _layoutWidth / 2;
    final centerY = _layoutHeight / 2;
    ...
    for (var i = 0; i < nodeOrderedList.length; i++) {
      ...
      final posX = cos(angle) * radius + centerX;
      final posY = sin(angle) * radius + centerY;
