...
class _CreateAutomatonScreenState extends State<CreateAutomatonScreen>
    ...
    try {
      Map<String, dynamic> automatonJson = {
        ...
        'transitions': _transitions.map(
          (from, transMap) => MapEntry(
            from,
            transMap.map(
              (symbol, toStates) =>
                  MapEntry(symbol, _isNFA ? toStates.toList() : toStates.first),
            ),
          ),
        ),
  ...
  Widget _buildTransitionsStep() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('تنظیم انتقال‌ها', style: Theme.of(context).textTheme.titleLarge),
        const SizedBox(height: 16),
        ..._states
            .map(
              (fromState) => Card(
  ...
  Widget _buildTransitionRow(String fromState, String symbol) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          SizedBox(width: 60, child: Text('$symbol →')),
          Expanded(
            child: Wrap(
              spacing: 4,
              children: [
