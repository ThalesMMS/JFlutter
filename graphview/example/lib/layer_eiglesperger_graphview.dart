...
class _LayeredEiglspergerGraphViewPageState
          ...
          children: [
            Wrap(
              children: [
                ...
                ElevatedButton(
                  onPressed: () {
                    final node12 = Node.Id(_nextNewNodeId++);
                    var edge =
  ...
  void _navigateToRandomNode() {
    ...
    final nodeIds = _availableNodeIds();
    if (nodeIds.isEmpty) return;

    final nodeId = nodeIds[r.nextInt(nodeIds.length)];
    _controller.animateToNode(nodeId);

    setState(() {
      nextNodeId = nodeId.value as int;
    });
  }

  List<ValueKey> _availableNodeIds() {
    return graph.nodes
        .map((node) => node.key)
        .whereType<ValueKey>()
        .where((key) => key.value is int)
        .toList();
  }

  int _maxNodeId() {
    return _availableNodeIds().map((key) => key.value as int).fold(0, max);
  }
  ...
  void initState() {
    ...
    final nodeIds = _availableNodeIds();
    _nextNewNodeId = _maxNodeId() + 1;
    nextNodeId = nodeIds[r.nextInt(nodeIds.length)].value as int;
  }
}
