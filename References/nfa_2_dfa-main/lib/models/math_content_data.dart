...
class SubsetContent extends SectionContentBase {
  ...
  List<ExampleItem> get examples => [
        ...
        ExampleItem(
          title: 'زیرمجموعه محض',
          problem:
              'A = {a, b} و B = {a, b, c} باشند. نوع رابطه بین A و B چیست؟',
...
class PowerSetContent extends SectionContentBase {
  ...
  List<ContentBlock> get theory => [
        ContentBlock(
          ...
          content:
              'مجموعه توانی مجموعه A، مجموعه‌ای از تمام زیرمجموعه‌های A است.',
          type: ContentType.definition,
          formula: 'P(A) = {S | S ⊆ A}',
        ),
        ContentBlock(
...
class FunctionContent extends SectionContentBase {
  ...
  Map<String, String> get definitions => {
        'تابع':
            'قاعده‌ای که به هر عنصر دامنه دقیقاً یک عنصر مدامنه نسبت می‌دهد',
...
class AchievementSystem {
  ...
  static List<Achievement> getNewAchievements(
    ...
    return newUnlocked
        .where((achievement) =>
            !oldUnlocked.any((old) => old.id == achievement.id))
        .toList();
  }
}
...
class PracticeManager {
  static List<QuizQuestion> getAdaptiveQuestions(
      int sectionId, ProgressTracker tracker,
      {int count = 5}) {
    final content = MathContentData.lessonSections[sectionId]?.content;
...
class MathLearningManager {
  ...
  int? getNextRecommendedSection() {
    ...
    for (var sectionId in MathContentData.lessonSections.keys.toList()
      ..sort()) {
