//
//  trace_persistence_service_io.dart
//  JFlutter
//
...
class TracePersistenceService {
  ...
  Future<void> saveTrace(SimulationResult trace, {String? customId}) async {
    try {
      ...
      await _prefs.setString(_currentTraceKey, jsonEncode(trace.toJson()));
  ...
  Future<SimulationResult?> loadCurrentTrace() async {
    try {
      final traceJson = _prefs.getString(_currentTraceKey);
  ...
  Future<void> setMaxHistorySize(int maxSize) async {
    try {
      await _prefs.setInt(_maxHistorySize, maxSize);
    } catch (e) {
  ...
  Future<int> getMaxHistorySize() async {
    try {
      return _prefs.getInt(_maxHistorySize) ?? _defaultMaxHistory;
    } catch (e) {
