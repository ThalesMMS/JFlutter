...
class _TreeViewPageState extends State<TreeViewPage>
  ...
  void _navigateToRandomNode() {
    ...
    setState(() {
      final currentIndex = nodeIds.indexOf(nodeId.value as int);
  ...
  int _nextUniqueNodeId() {
    final nodeIds = _availableNodeIds();
    return nodeIds.isEmpty ? 1 : nodeIds.reduce(max) + 1;
  }

  List<int> _availableNodeIds() {
    return graph.nodes.map((node) => node.key?.value).whereType<int>().toList()
      ..sort();
  }
