...
class RegexToNFAConverter {
  ...
  static FSA _thompsonConstructionWithSteps(
    RegexNode node,
  ...
  static FSA _buildNFAWithSteps(
    ...
    switch (node) {
      case EpsilonNode():
        ...
        final transition = nfa.fsaTransitions.first;
        steps.add(
          RegexToNFAStep.basicSymbol(
            id: 'step_${stepCounter + steps.length}',
            stepNumber: stepCounter + steps.length,
            symbol: 'ε',
            position: 0,
            startState: startState,
            acceptState: acceptState,
            transition: transition,
            stackSize: steps.length + 1,
          ),
        );
        return nfa;
      ...
      case UnionNode(:final left, :final right):
        final leftNFA = _buildNFAWithSteps(
          left,
          steps,
          stepCounter,
          contextAlphabet: contextAlphabet,
        );
        final rightNFA = _buildNFAWithSteps(
          right,
          steps,
          stepCounter,
          contextAlphabet: contextAlphabet,
        ...
        steps.add(
          RegexToNFAStep.union(
            ...
            firstAccept: leftNFA.acceptingStates.first,
            secondStart: rightNFA.initialState!,
            secondAccept: rightNFA.acceptingStates.first,
            newTransitions: newTransitions,
            stackSize: steps.length,
          ),
        );
        return nfa;
      ...
      case ConcatenationNode(:final left, :final right):
        ...
        final nfa = _buildConcatenationNFA(
          left,
          right,
          contextAlphabet: contextAlphabet,
        );
      ...
      case KleeneStarNode(:final child):
        final childNFA = _buildNFAWithSteps(
          child,
          steps,
          stepCounter,
          contextAlphabet: contextAlphabet,
        ...
        final newTransitions = nfa.fsaTransitions
            .where(
              (t) =>
                  t.fromState == newStart ||
                  t.toState == newAccept ||
                  childNFA.acceptingStates.contains(t.fromState),
            )
            .toSet();
        ...
        steps.add(
          RegexToNFAStep.kleeneStar(
            ...
            oldAccept: childNFA.acceptingStates.first,
            newTransitions: newTransitions,
            stackSize: steps.length,
          ),
        );
      ...
      case PlusNode(:final child):
        ...
        final nfa = _buildPlusNFA(child, contextAlphabet: contextAlphabet);
        final newStart = nfa.initialState!;
        final newAccept = nfa.acceptingStates.first;
        final newTransitions = nfa.fsaTransitions
            .where(
      ...
      case QuestionNode(:final child):
        ...
        final nfa = _buildQuestionNFA(child, contextAlphabet: contextAlphabet);
        final newStart = nfa.initialState!;
        final newAccept = nfa.acceptingStates.firstWhere((s) => s != newStart);
        final newTransitions = nfa.fsaTransitions
            .where(
  ...
  static FSA _buildConcatenationNFA(
    RegexNode left,
    RegexNode right, {
    Set<String>? contextAlphabet,
  }) {
  ...
  static FSA _buildKleeneStarNFA(
    RegexNode child, {
    Set<String>? contextAlphabet,
  }) {
  ...
  static FSA _buildPlusNFA(RegexNode child, {Set<String>? contextAlphabet}) {
    ...
    final childNFA = _buildNFA(child, contextAlphabet: contextAlphabet);
    final kleeneNFA = _buildKleeneStarNFA(
      child,
      contextAlphabet: contextAlphabet,
    );
