...
class CYKParser {
  static Result<CYKResult> parse(Grammar g, String input) {
    try {
      if (input.isEmpty) {
        final acceptsEmpty = g.productions.any((p) => p.isLambda && p.leftSide.first == g.startSymbol);
        return ResultFactory.success(CYKResult(
          accepted: acceptsEmpty,
          table: const [],
          derivation: acceptsEmpty ? CYKDerivation.node(g.startSymbol, []) : null,
        ));
      }
      final n = input.length;
      final table = List.generate(n, (i) => List.generate(n, (j) => <String>{}));
      final back = List.generate(n, (i) => List.generate(n, (j) => <String, CYKBackptr>{}));
      ...
      for (final p in g.productions) {
        ...
        if (p.rightSide.length == 1 && g.terminals.contains(p.rightSide.first)) {
          final a = p.rightSide.first;
          (unary[a] ??= <String>{}).add(p.leftSide.first);
        } else if (p.rightSide.length == 2 &&
            g.nonterminals.contains(p.rightSide[0]) && g.nonterminals.contains(p.rightSide[1])) {
      ...
      return ResultFactory.success(CYKResult(accepted: accepted, table: table, derivation: tree));
    } catch (e) {
      return ResultFactory.failure('CYK parse error: $e');
    }
  }

  static CYKDerivation _buildTree(List<List<Map<String, CYKBackptr>>> back, int i, int j, String A) {
    final bp = back[i][j][A];
    if (bp == null) return CYKDerivation.node(A, []);
    if (bp.isLeaf) return CYKDerivation.node(A, [CYKDerivation.leaf(bp.leafSymbol!)]);
...
class CYKResult {
  final bool accepted;
  final List<List<Set<String>>> table; // upper triangular table; table[i][len-1]
  final CYKDerivation? derivation;
  const CYKResult({required this.accepted, required this.table, required this.derivation});
}
...
class CYKDerivation {
  ...
  factory CYKDerivation.node(String nt, List<CYKDerivation> children) => CYKDerivation._(nt, children);
...
class CYKBackptr {
  ...
  factory CYKBackptr.internal({required (int,int,String) left, required (int,int,String) right}) =>
