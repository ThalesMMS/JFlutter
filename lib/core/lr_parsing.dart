...
class LRParsing {
  ...
  static Set<String> _calculateFirstForString(Map<String, Set<String>> firstSets, String string) {
    ...
    // Special case: if string is just the end marker, return it
    if (string == '\$') {
      result.add('\$');
      return result;
    }
    
    bool allHaveLambda = true;
    ...
    // If all symbols have λ in their FIRST set, add λ
    if (allHaveLambda) {
      result.add('λ');
    }
  ...
  /// Helper function to check if two sets of LR items are equal
  static bool _itemsSetsEqual(Set<LRItem> set1, Set<LRItem> set2) {
    if (set1.length != set2.length) return false;
    for (final item1 in set1) {
      bool found = false;
      for (final item2 in set2) {
        if (item1 == item2) {
          found = true;
          break;
        }
  ...
  /// Helper function to find existing state for a set of items
  static int? _findExistingState(Map<int, Set<LRItem>> stateToItems, Set<LRItem> items) {
    for (final entry in stateToItems.entries) {
      if (_itemsSetsEqual(entry.value, items)) {
        return entry.key;
      }
    }
    return null;
  ...
  static Map<int, LRState> buildLRAutomaton(ContextFreeGrammar grammar) {
    ...
    while (queue.isNotEmpty) {
      ...
      // Check if this set of items already exists
      final existingStateId = _findExistingState(stateToItems, currentItems);
      ...
      for (final symbol in symbols) {
        ...
        if (gotoItems.isNotEmpty) {
          // Check if this goto set already exists
          final existingGotoStateId = _findExistingState(stateToItems, gotoItems);
          if (existingGotoStateId == null) {
            queue.add(gotoItems);
          }
        }
      }
  ...
  static LRParseTable generateParseTable(ContextFreeGrammar grammar) {
    final augmentedGrammar = createAugmentedGrammar(grammar);
    final states = buildLRAutomaton(grammar);
    ...
    // Create stateToItems mapping
    final stateToItems = <int, Set<LRItem>>{};
    for (final stateEntry in states.entries) {
      stateToItems[stateEntry.key] = stateEntry.value.items;
    }
    ...
    for (final stateEntry in states.entries) {
      ...
      for (final item in state.items) {
        if (item.isReduceItem) {
        ...
        } else if (item.rhs == 'λ' && item.dotPosition == 0) {
          // Lambda production - add reduce action immediately
          int productionNumber = -1;
          for (int i = 0; i < augmentedGrammar.productions.length; i++) {
            final prod = augmentedGrammar.productions[i];
            if (prod.leftHandSide == item.lhs && prod.rightHandSide == 'λ') {
              productionNumber = i;
              break;
            }
          }
          ...
          if (symbolAfterDot != null) {
            ...
            final targetStateId = _findExistingState(stateToItems, gotoItems);
    ...
    while (true) {
      ...
      switch (action.action) {
        ...
        case 'r':
          ...
          // Get goto state
          if (stack.isEmpty) {
            steps.add('ERRO: Stack vazia após reduce');
            return LRParsingResult(
              accepted: false,
              steps: steps,
              explanation: 'Erro de parsing: stack vazia',
              derivation: derivation,
            );
          }
