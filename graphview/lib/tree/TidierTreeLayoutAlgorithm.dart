...
class TidierTreeLayoutAlgorithm extends Algorithm {
  ...
  void _buildTree(Graph graph) {
    ...
    final layoutTree = _createSpanningForest(graph);
    _initializeData(layoutTree);
    roots = _findRoots(layoutTree);
    tree = layoutTree;
    ...
    _normalizePositions(layoutTree);
  }
  ...
  Graph _createSpanningForest(Graph graph) {
    ...
    final outgoingEdges = <Node, List<Edge>>{};
    final incomingCounts = <Node, int>{};
    for (final node in graph.nodes) {
      outgoingEdges[node] = [];
      incomingCounts[node] = 0;
    ...
    for (final edge in graph.edges) {
      outgoingEdges.putIfAbsent(edge.source, () => []).add(edge);
      incomingCounts[edge.destination] =
          (incomingCounts[edge.destination] ?? 0) + 1;
    }

    final roots = graph.nodes
        .where((node) => incomingCounts[node] == 0)
        .toList(growable: false);
    final startNodes = roots.isEmpty ? graph.nodes : [...roots, ...graph.nodes];
    ...
    for (final startNode in startNodes) {
      ...
      while (queue.isNotEmpty) {
        ...
        for (final edge in outgoingEdges[current] ?? const <Edge>[]) {
          final child = edge.destination;
          if (visited.contains(child)) continue;

          forest.addEdgeS(edge);
          visited.add(child);
          queue.add(child);
        }
      }
